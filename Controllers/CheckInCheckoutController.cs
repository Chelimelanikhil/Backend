    ...
    public async Task<IActionResult> CheckInOrOut([FromBody] CheckinCheckout request)
    {
        ...
        try
        {
            ...
            if (request.RequestType == "checkin")
            {
                ...
                var checkinCheckoutMgr = new CheckinCheckoutMgr(_configuration);
                result = await checkinCheckoutMgr.HandleCheckinAsync(request);
            }
            else if (request.RequestType == "checkout")
            {
                var checkinCheckoutMgr = new CheckinCheckoutMgr(_configuration);
                result = await checkinCheckoutMgr.HandleCheckoutAsync(request);
            }
            else
            {
                return BadRequest("Invalid request type.");
